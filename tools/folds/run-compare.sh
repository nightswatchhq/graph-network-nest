...
spike=$here/spike ckpt=${SPIKE_CKPT:-$HOME/spike-0059/ckpt-a}
res=${RESULTS:-results-$tag}; mkdir -p "$res"
: > "$res/projection.log"
for n in $(cat "$spike/diff.blocks") 450001618 500000152; do
    got=$("$here/measure.sh" "read-$n" "$here/project.sh" "$nest" "$n" "$res/out/$n" 2> "$res/out-$n.err")
    exp=$(jq -S -c . "$here/ref/spikefold-$n.json")
    v=MATCH; [ "$got" = "$exp" ] || v=MISMATCH
    b=MISSING; rb=$spike/out/ref-$n-bounds.csv
...
cmp -s "$spike/gw-123m-bounds.csv" "$res/out/123615752/bounds.csv" \
...
for n in ${CARRY_BLOCKS:-50000000 100000000 150000000 200000000 250000000 300000000 350000000 400000000 450000000 500000000 506179123 507079123 507177123}; do
    "$here/carries.sh" "$nest" "$n" "$ckpt/$n" "$res/carries/$n" >> "$res/carries.log" 2>&1
done
