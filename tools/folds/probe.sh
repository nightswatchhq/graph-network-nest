...
sp=$here/spike ckpt=${SPIKE_CKPT:-$HOME/spike-0059/ckpt-a}
source_nest=${SOURCE_NEST:-$HOME/.local/state/network-facade/nest-parity-20260920}
res=${RESULTS:-results-$tag}; mkdir -p "$res/probe"
: > "$res/probe.log"
...
for c in $cuts; do
    ...
    { echo "SET temp_directory='$res/probe/tmp-$c';"; cat $sp/macros.sql
      $sp/gen-raw.sh "$source_nest" "$c" $((c + 5000000))
    ...
    got=$("$here/project.sh" "$nest" "$n" "$res/probe/$n" 2> "$res/probe/$n.err")
    exp=$($sp/eval-at.sh "$ckpt" "$n" 2>/dev/null | jq -S -c .)
