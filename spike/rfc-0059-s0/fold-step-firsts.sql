...
SELECT block_number, log_index, l1_block FROM (
    ...
    FROM w_obs
) first_epoch WHERE ordinal = 1 AND epoch NOT IN (SELECT epoch FROM c_epochs)
UNION ALL
SELECT block_number, log_index, l1_block FROM (
...
-- Probe only: the first observation of each epoch that begins in this window.
COPY (
    SELECT epoch, block_number, log_index FROM w_obs
    WHERE epoch NOT IN (SELECT epoch FROM c_epochs)
    QUALIFY row_number() OVER (PARTITION BY epoch ORDER BY block_number, log_index) = 1
    ORDER BY epoch
) TO '__OUT__/epoch_firsts.csv' (FORMAT csv, HEADER);
