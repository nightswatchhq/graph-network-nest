# Graph Network Nest - IN PROGRESS

> **Status: draft, not deployable, not a replacement for graph-node or the Network Subgraph.**
>
> Work stopped on 2026-09-21 after a complete Arbitrum One replay reached head but the native
> GraphQL serving path could not meet the public endpoint's latency and memory budget. The public
> `https://network.thenightswatch.dev/graphql` route remains a rate-limited proxy to The Graph's
> grafted Network Subgraph, not this nest. Do not point production indexer-agent,
> indexer-service-rs or TAP workloads at a Nuthatch listener from this repository.
>
> The useful artefacts here are the contract inventory, pinned-call rules, event folds, captured
> client documents and parity fixtures. The remaining engineering is maintained current state for
> GraphQL roots, while block-pinned queries continue to use the immutable archive. Until that is
> implemented and independently proven against the gateway, this is research code.
